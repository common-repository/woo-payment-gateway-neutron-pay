<?php
namespace Neutronpay;

class Neutronpay
{
    const VERSION           = '1.0.0';
    const USER_AGENT_ORIGIN = 'Neutronpay PHP Library';
    ...
    public static function request($url, $method = 'POST', $params = array(), $authentication = array())
    {
        ...
        if (empty($auth_token))
            \Neutronpay\Exception::throwException(400, array('reason' => 'CredentialsMissing', 'message' => 'Set up your credentials on plugin\'s settings'));
        ...
        if (!in_array($environment, $environments)) {
            ...
            \Neutronpay\Exception::throwException(400, array('reason' => 'BadEnvironment', 'message' => "Environment does not exist. Available environments: $availableEnvironments"));
        }

        $url       = ($environment === 'live' ? 'https://api.neutronpay.com' : 'https://sandboxapi.neutronpay.com') . $url;
        $headers   = array();
        ...
        if (array_key_exists('data', $response)){
            ...
            \Neutronpay\Exception::throwException($http_status, $response);
        }
    }
}